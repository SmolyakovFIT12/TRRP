    ...
    public class SocketClient
    {
        ...
        public SocketClient(string host, int port)
        {
            ...
            while (!socket.Connected)
            {
                try
                {
                    Console.WriteLine("Socket Сервер: попытка подключиться к серверу...");
                    socket.Connect(new IPEndPoint(IPAddress.Parse(host), 80));
                    Console.WriteLine("Socket Сервер: соединение установлено");
                    break;
                }
                catch (Exception e)
                {
                    Console.WriteLine("Socket Сервер: не удалось подключиться к сокет серверу " + e.Message);
                    Console.WriteLine("Socket Сервер: повторная попытка через 10 секунд ");
                }
                Thread.Sleep(10000);
            }
            startReceiving();
